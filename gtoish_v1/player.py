...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        if street == 0:
            ...
            if ((my_preflop_range in self.pre_flop_cards_all) or (my_preflop_range_alt in self.pre_flop_cards_all)) and RaiseAction in legal_actions:
                ...
                if max_cost <= my_stack: #make sure the max_cost is something we can afford! Must have at least this much left after our other costs
                    my_action = RaiseAction(math.floor(min_raise*5)) #GO ALL IN!!!
        ...
        if hand_strength_percentage > .9 and RaiseAction in legal_actions: # all-in
            raise_amount = math.floor(.5*my_stack)
        ...
        if hand_strength_percentage > .55 and street == 3 and RaiseAction in legal_actions: # aggressive flop
            raise_amount = math.floor(hand_strength_percentage/5*my_stack)
            # # ensure raises are legal
            raise_amount = max([min_raise, raise_amount]) #getting the max of the min raise and the raise amount
            raise_amount = min([max_raise, raise_amount]) #getting the min of the max raise and the raise amount
            return RaiseAction(raise_amount)
        if hand_strength_percentage > .75 and street == 4 and RaiseAction in legal_actions: # aggressive turn
            raise_amount = math.floor(hand_strength_percentage/3*my_stack)
            # # ensure raises are legal
            raise_amount = max([min_raise, raise_amount]) #getting the max of the min raise and the raise amount
            raise_amount = min([max_raise, raise_amount]) #getting the min of the max raise and the raise amount
            return RaiseAction(raise_amount)
        if hand_strength_percentage > (continue_cost/(continue_cost + pot_total)) and CallAction in legal_actions:
            return CallAction()
